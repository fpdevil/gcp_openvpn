...
script_log="setup_ovpn_log_`date +%F`.log"
exec 1>>$script_log
exec 2>&1
...
echo "clearing cache..."
sudo rm -r /var/lib/apt/lists/*             # Clear out the corrupted files
sudo rm /var/cache/apt/archives/lock

echo "reconfiguring the state..."
sudo dpkg --configure -a

echo "Updating/Upgrading the packages..."
# sudo flock /var/lib/apt/lists/lock -c "apt-get update"
# sudo apt-get -o DPkg::Lock::Timeout=3 dist-upgrade
sudo apt-get clean                          # Clean the local repository of retrieved package files
sudo mkdir -p /var/lib/apt/lists/partial    # Create the partial directory
sudo apt-get update -y                      # Re-sync the package index
